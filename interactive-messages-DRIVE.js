...
// ============================================
// ÜRÜN LİSTESİ (Drive'dan) - BUTTONS
// ============================================
async function sendProductList(phoneNumber, categoryName) {
  ...
  // Her ürün için text oluştur
  const productList = availableProducts.map((p, index) => 
    `${index + 1}. *${p.name}*\n   ${p.description || ''}\n   💰 ${p.price}`
  ).join('\n\n');
  ...
  const data = {
    ...
    interactive: {
      type: 'button',
      body: {
        text: `*${categoryName.toUpperCase()} KATEGORİSİ*\n\n${productList}\n\nSepete eklemek için ürün numarasını yazın.`
      },
      action: {
        buttons: [
          {
